...
'''
相比原版的修改：
将MSA替换为HR-MSA，这个操作很容易，这是由于CRAFT采用原版的MSA
'''
...
class Attention_regular(nn.Module):
    ...
    def forward(self, qkv, H, W, mask=None, rpi=None, rpe_biases=None):
        ...
        q = self.im2win(q, H, W) # [B,h,n,c]
        k = self.im2win(k, H, W)
        v = self.im2win(v, H, W) 
        # q = q * self.scale
        # 在这里添加HRMSA
        b,h,n,c = q.shape
        q = q.permute(0,2,1,3).reshape(b,n,h*c)
        k = k.permute(0,2,1,3).reshape(b,n,h*c)
        
        B_q, B_k, C_q, C_k = self.get_B_C(q,k)
        attn = ((q @ k.transpose(-2,-1)).unsqueeze(1)
                * C_q * C_k
                + B_q + B_k
                ) * self.scale 
...
class HRMSA_CRAFT(nn.Module):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(HRMSA_CRAFT, self).__init__()

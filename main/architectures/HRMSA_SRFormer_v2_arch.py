...
使用HRMSA
注意：
这个版本的修改方案是：将x当做q（即删除q对应的参数量），然后保留k的参数，
但由于采用permate方法，2*2窗口的k会直接并联合并在一起，这2*2个像素对应的K的可训练参数是一致的，但是由于本身应该是有位置区别的，
因此对这4个像素采用不同的可训练参数应该是合理的，这将在v2进行修改，用于作对照。
...
class linear_groups(nn.Module):
    def __init__(self, indim,outdim,groups,*args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self.outdim = outdim
        self.body = nn.Conv2d(indim,outdim,1,groups=groups)
    def forward(self,x):
        b_,n,c = x.shape
        x=x.reshape(b_*n,c,1,1)
        out = self.body(x).reshape(b_,n,self.outdim)
        return out

class PSA_HRMSA(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.,
        ...
        self.k = linear_groups(indim=dim*4,outdim=dim,groups=4)
        self.v = nn.Linear(dim, dim//4, bias=qkv_bias)
    ...
    def forward(self, x, mask=None):
        ...
        # kv = self.kv(x) # b_,n,c/2
        v = self.v(x) # b_,n,c/4
        v = v.reshape(b_,self.permuted_window_size[0],2,self.permuted_window_size[1],2,1,c//4).permute(0,1,3,5,2,4,6)
        v = v.reshape(b_, n//4, 1, self.num_heads, c // self.num_heads).permute(2, 0, 3, 1, 4)[0] # [B_,h,n//4,nh]
        k = x.reshape(b_,self.permuted_window_size[0],2,self.permuted_window_size[1],2,1,c).permute(0,1,3,5,2,4,6)
        # kv = kv.reshape(b_, n//4, 2, self.num_heads, c // self.num_heads).permute(2, 0, 3, 1, 4)
        k = k.reshape(b_, n//4, c*4)
        k = self.k(k)
        # keep the channel dimension of Q: (num_windows*b, num_heads, n, c//num_heads)
        # attn = (q @ k.transpose(-2, -1))* self.scale   # (num_windows*b, num_heads, n, n//4)
        B_q, B_k, C_q, C_k = self.get_B_C(x,k)
        attn = ((x @ k.transpose(-2,-1)).unsqueeze(1)# [b_,1,n,n//4]
                * C_q * C_k
                + B_q + B_k
                ) * self.scale # [b_,nh,n,n//4]
...
class HRMSA_SRFormer_v2(nn.Module):
    r""" SRFormer
    ...
    def __init__(self,
                 ...
                 upsampler='',
                 resi_connection='1conv',
                 magnification = 1, # 这个参数是历史遗留问题，已经被弃用
                 **kwargs):
        super(HRMSA_SRFormer_v2, self).__init__()
        self._input_shape_for_flops = (1,3,64,64)
        num_in_ch = in_chans
        num_out_ch = in_chans
        num_feat = 64

...
class PSA(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.,
        ...
        self.head_dim = int(dim * magnification) // num_heads 
        ...
        self.q = nn.Linear(dim,int(dim*self.mag),bias=qkv_bias)
        self.k = nn.Linear(dim, int(dim*self.mag)//4, bias=qkv_bias)
    ...
    def forward(self, x, mask=None):
        ...
        k,v = self.k(x),self.v(x) # b_,n,c/4
        # kv = kv.reshape(b_,self.permuted_window_size[0],2,self.permuted_window_size[1],2,2,c//4).permute(0,1,3,5,2,4,6)
        k = k.reshape(b_,self.permuted_window_size[0],2,self.permuted_window_size[1],2,1,int(c*self.mag)//4).permute(0,1,3,5,2,4,6)
        ...
        k = k.reshape(b_, n//4, 1, self.num_heads, int(c*self.mag) // self.num_heads).permute(2, 0, 3, 1, 4)[0] # [B_,h,n//4,nh*m]
        ...
        q = self.q(x).reshape(b_, n, 1, self.num_heads, int(c*self.mag) // self.num_heads).permute(2, 0, 3, 1, 4)[0] # [b_,h,n,hd*m]
        q = q * self.scale
        attn = (q @ k.transpose(-2, -1))   # (num_windows*b, num_heads, n, n//4)

        global calculate_rank
        calculate_rank.calculate_rank(mat=attn.reshape(-1,n,n//4))
        calculate_rank.calculate_average_entropy(mat=attn.reshape(-1,n,n//4))
...
class final_SRFormer_magnification(nn.Module):
    r""" SRFormer
    ...
    def __init__(self,
                 ...
                 magnification = 1,
                 **kwargs):
        super(final_SRFormer_magnification, self).__init__()
...
if __name__ == '__main__':
    weight_path = 'weights/SRFormer_mag4_DIV_x2_net_g_latest.pth'
    input_path = 'datasets/img_092.png'
    net = final_SRFormer_magnification(
        upscale= 2,
        in_chans= 3,
        img_size= 64,
        window_size= 16,
